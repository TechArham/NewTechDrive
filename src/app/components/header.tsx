...
export default function Header({ variant = 'light' }: HeaderProps) {
    ...
    const servicesTimeoutRef = useState<NodeJS.Timeout | null>(null);

    const handleServicesEnter = () => {
        if (servicesTimeoutRef[0]) {
            clearTimeout(servicesTimeoutRef[0]);
            servicesTimeoutRef[1](null);
        }
            ...
            <div className={`w-full h-16 md:h-20 ${bgClass} rounded-full mt-2 md:mt-4 mx-auto max-w-[1400px] flex items-center justify-between px-4 md:px-8 transition-all duration-300`}>
                ...
                <div className="h-full flex items-center">
                    <Link href="/" className="hover:opacity-80 transition-opacity">
                        <Image src={logoSrc} alt="Tech Drive Logo" width={300} height={150} className="w-auto h-12 md:h-14 object-contain" priority />
                    </Link>
                </div>

                {/* Middle: Navigation Links (Desktop) */}
                <nav className={`hidden lg:flex items-center gap-8 ${textClass} font-semibold text-[15px]`}>
                    {/* Services Dropdown */}
                    <div
                        className="relative"
                    ...
                    >
                        ...
                        {isServicesOpen && (
                            <div
                                className={`absolute top-full left-0 mt-1 w-64 ${isDark ? 'bg-white/10 backdrop-blur-xl border border-white/20' : 'bg-white border border-gray-100'} rounded-2xl shadow-xl py-2 overflow-hidden animate-fadeIn z-50`}
                                onMouseEnter={handleServicesEnter}
                                onMouseLeave={handleServicesLeave}
                            >
                ...
                <div className="flex lg:hidden items-center gap-3">
                    <button
                        ...
                        className={`w-10 h-10 rounded-full cursor-pointer ${isDark ? 'border border-white/30 text-white hover:bg-white/10' : 'border border-gray-200 text-gray-600 hover:bg-gray-50'} flex items-center justify-center transition-all duration-300 hover:scale-105 focus:outline-none`}
                    >
